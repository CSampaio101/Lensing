...
def Point(x1,x2,x1l,x2l,ml): # Point lens of mass ml at x1l,x2l
    ...
    y1=x1-ml*(x1ml)/d # Lens equation for x coordinate
    y2=x2-ml*(x2ml)/d # Lens equation for y coordinate
    return (y1,y2)

def TwoPoints(x1,x2,x1l1,x2l1,x1l2,x2l2,ml1,ml2): #Two diff lenses.
    x1ml1=(x1-x1l1)
    x2ml1=(x2-x2l1)
    x1ml2=(x1-x1l2)
    x2ml2=(x2-x2l2)
    ...
    y1=x1-ml1*(x1ml1)/d1-ml2*(x1ml2)/d2 # Lens equation for x coordinate
    y2=x2-ml1*(x2ml1)/d1-ml2*(x2ml2)/d2
    return (y1,y2)

def ChangRefsdal(x1,x2,x1l,x2l,ml,k,g): #This one accounts for gravitational field
    x1ml=(x1-x1l)
    x2ml=(x2-x2l)
...
def SIS(x1,x2,x1l,x2l,k): #This is the isothermal sphere
    x1ml=(x1-x1l)
    x2ml=(x2-x2l)
